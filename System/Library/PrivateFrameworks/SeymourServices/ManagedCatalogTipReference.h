//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <CoreData/NSManagedObject.h>

@class NSOrderedSet, NSString;

@interface ManagedCatalogTipReference : NSManagedObject
{
}

- (id)initWithEntity:(id)arg1 insertIntoManagedObjectContext:(id)arg2;	// IMP=0x0000000000616b40

// Remaining properties
@property(nonatomic, retain) NSOrderedSet *categoryIdentifiers; // @dynamic categoryIdentifiers;
@property(nonatomic, copy) NSString *dateReleased; // @dynamic dateReleased;
@property(nonatomic, copy) NSString *identifier; // @dynamic identifier;

@end

