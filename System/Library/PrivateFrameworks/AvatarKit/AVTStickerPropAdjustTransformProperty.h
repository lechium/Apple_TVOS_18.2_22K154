//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class MISSING_TYPE;

__attribute__((visibility("hidden")))
@interface AVTStickerPropAdjustTransformProperty : NSObject
{
    long long _presetCategory;	// 8 = 0x8
    id _presetValue;	// 16 = 0x10
    MISSING_TYPE *_position;	// 32 = 0x20
    MISSING_TYPE *_rotation;	// 48 = 0x30
    MISSING_TYPE *_scale;	// 64 = 0x40
}

- (void).cxx_destruct;	// IMP=0x0000000000031d2a
@property(readonly, nonatomic) MISSING_TYPE *scale; // @synthesize scale=_scale;
@property(readonly, nonatomic) MISSING_TYPE *rotation; // @synthesize rotation=_rotation;
@property(readonly, nonatomic) MISSING_TYPE *position; // @synthesize position=_position;
@property(readonly, nonatomic) id presetValue; // @synthesize presetValue=_presetValue;
@property(readonly, nonatomic) long long presetCategory; // @synthesize presetCategory=_presetCategory;
- (void)applyAdjustmentIfNeededToNode:(id)arg1 forMemoji:(id)arg2 ignoreScaleZ:(_Bool)arg3;	// IMP=0x0000000000031ac4
- (id)initWithPresetCategory:(long long)arg1 presetValue:(id)arg2 position:rotation:scale: /* Error: Ran out of types for this method. */;	// IMP=0x0000000000031a23

@end

