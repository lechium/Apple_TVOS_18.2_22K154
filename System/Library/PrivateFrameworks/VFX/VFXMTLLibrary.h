//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSURL, VFXMTLLibraryManager;
@protocol MTLLibrary;

@interface VFXMTLLibrary : NSObject
{
    NSURL *_libraryURL;	// 8 = 0x8
    VFXMTLLibraryManager *_manager;	// 16 = 0x10
    id <MTLLibrary> _library;	// 24 = 0x18
}

@property(readonly, nonatomic) id <MTLLibrary> library; // @synthesize library=_library;
- (void)_load;	// IMP=0x000000000019e444
- (void)dealloc;	// IMP=0x000000000019e3f3
- (id)initWithPath:(id)arg1 manager:(id)arg2;	// IMP=0x000000000019e35f

@end

