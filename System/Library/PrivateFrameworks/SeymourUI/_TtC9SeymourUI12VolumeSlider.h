//
//     Generated by classdump-c 4.2.0 (64 bit) (iOS port by DreamDevLost, Updated by Kevin Bradley.)(Debug version compiled Dec 27 2024 14:18:54).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <UIKitCore/UIView.h>

@class MISSING_TYPE, NSString, UIWindowScene;

__attribute__((visibility("hidden")))
@interface _TtC9SeymourUI12VolumeSlider : UIView
{
    MISSING_TYPE *onVolumeChanged;	// 8 = 0x8
    MISSING_TYPE *sliderView;	// 24 = 0x18
    MISSING_TYPE *stackView;	// 32 = 0x20
    MISSING_TYPE *styleProvider;	// 40 = 0x28
    MISSING_TYPE *timerProvider;	// 96 = 0x60
    MISSING_TYPE *isTracking;	// 136 = 0x88
    MISSING_TYPE *route;	// 144 = 0x90
    MISSING_TYPE *volumeCommitTimer;	// 152 = 0x98
    MISSING_TYPE *volumeController;	// 160 = 0xa0
}

- (void).cxx_destruct;	// IMP=0x000000000004f8c0
- (void)didMoveToWindow;	// IMP=0x000000000004f020
- (id)initWithCoder:(id)arg1;	// IMP=0x000000000004ef50
- (id)initWithFrame:(struct CGRect)arg1;	// IMP=0x000000000004ef20
- (void)volumeController:(id)arg1 volumeValueDidChange:(float)arg2;	// IMP=0x000000000004f9e0
- (void)volumeController:(id)arg1 volumeControlAvailableDidChange:(_Bool)arg2;	// IMP=0x000000000004f990
@property(nonatomic, readonly) UIWindowScene *windowSceneForVolumeDisplay;
@property(nonatomic, readonly) NSString *volumeAudioCategory;
- (_Bool)isOnScreenForVolumeDisplay;	// IMP=0x000000000004fa30

// Remaining properties
@property(nonatomic, readonly) _Bool onScreenForVolumeDisplay;

@end

